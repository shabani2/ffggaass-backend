...
import aggregateLivraisons from '../Controllers/livraisonAggregation.js'; // Assurez-vous que le chemin est correct
import SituationLivraison from '../Models/SituationLivraisonSchema.js'; // Assurez-vous que le chemin est correct
import mongoose from 'mongoose';

const rvsLivraisonRoute = express.Router();

// Route pour effectuer l'agrégation et enregistrer les résultats
rvsLivraisonRoute.post('/aggregate-and-save', async (req, res) => {
  try {
    const { pointVente, produit, dateStart, dateEnd } = req.body;

    // Effectuer l'agrégation
    const aggregatedData = await aggregateLivraisons(pointVente, produit, dateStart, dateEnd);
    ...
    await SituationLivraison.insertMany(aggregatedData);
...
rvsLivraisonRoute.get('/get-situations', async (req, res) => {
    try {
      ...
      const situations = await SituationLivraison.find(query)
...
// Filtre par pointVenteId
rvsLivraisonRoute.get('/filter-by-pointVente', async (req, res) => {
  try {
    const { pointVenteId } = req.query; // Récupère pointVenteId depuis les paramètres de requête
    console.log('pointVenteId:', pointVenteId);
    const situations = await SituationLivraison.find({
...
rvsLivraisonRoute.get('/filter-not-by-pointVente', async (req, res) => {
  try {
    const { pointVenteId } = req.query; // Récupère pointVenteId depuis les paramètres de requête
    console.log('pointVenteId:', pointVenteId);
...
// Filtrer par produit
rvsLivraisonRoute.get('/filter-by-produit', async (req, res) => {
    try {
      const { produit } = req.query;
  
      const situations = await SituationLivraison.find({ produit })
        .populate('produit')
        .populate('pointVente');
...
export default rvsLivraisonRoute;
